...
func floor_pass(path, level, floor_set):  #param: path generated by walker  #param: level file containg data rep of the level  #param: floor tile set tile set that is being written to
	for step in path:
...
func room_deco_pass(room_list, path_by_rooms, floor_set):
...
func wall_pass(path, level, wall_set):  #param: path generated by walker  #param: level file containg data rep of the level  #param: wall tile set that is being written to
	var processed = {}
	for step in path:
		for neighbour in neighbours_kernel:
			var curr = Vector2(
				step.x + neighbours_kernel[neighbour].x, step.y + neighbours_kernel[neighbour].y
			)
			...
			elif processed.get(curr) == null and level[curr.x][curr.y] == -1:
