...
func _ready():
	...
	if Engine.editor_hint:
		print(
			"Loaded Dialogue System in {time} usec.".format(
				{"time": Time.get_ticks_usec() - init_time}
			)
		)
...
func display_dialogue(npc_id: String, dialogue_id: String, bubble = false) -> void:
	if Dialogic.timeline_exists(npc_id + "-" + dialogue_id):
		...
		push_error(
			"Unknown dialogue {d_id} for npc {n_id}".format({"d_id": dialogue_id, "n_id": npc_id})
		)
	...
	match s_name:
		"pause":
			...
			$CanvasLayer/Tween.interpolate_property(
				...
				"modulate",
				$CanvasLayer/ColorRect.modulate,
				Color(1, 1, 1, 0.5),
				0.2
			)
...
func choice_selected(choice_id: String, option_selected: int) -> void:
	var key := "choice-{choice_id}-{option}".format(
		{"choice_id": choice_id, "option": option_selected}
	)
	_lookup_unlock_table(key)
